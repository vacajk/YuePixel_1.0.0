WebServer webServer(80); // WebServer对象，对象名称为webServer 网络服务器标准http端口号为80，因此这里使用80为端口号
...
{
    webServer.begin();
    webServer.on("/", handleRoot);  //开启各网页服务
    webServer.on("/pic", handlePic);
    webServer.on("/time", handleTime);
    webServer.on("/words", handleWords);
    webServer.on("/wordsetting", handleWordsetting);
    webServer.on("/animation", handleAnimation);
...
{                                                             // 当浏览器请求的网络资源无法在服务器找到时，
    webServer.send(404, "text/plain", "404: Not found"); // NodeMCU将调用此函数。
}
